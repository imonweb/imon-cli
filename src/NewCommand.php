<?php
/**
 * Generate a new installation of Imon
 *
 * @category CLI
 * @package  Imon-cli
 * @author   Imon Dela Rosa <contact@imonweb.co.uk>
 * @license  http://opensource.org/licenses/gpl-3.0 GNU General Public License, version 3 (GPLv3)
 * @link     https://www.imonweb.co.uk
 */
...
class NewCommand extends Command
{
	...
	public function configure()
	{
		$this
			->setName('new')
			->setDescription('Create a new IMON Theme installation')
	...
	public function execute(InputInterface $input, OutputInterface $output)
	{
		...
		$question = new Question("Name of your theme? <info>(Imon)</info> ", null);
		$themeName = $helper->ask($input, $output, $question);

		$question = new Question("PHP Namespace of your theme? <info>(Imon)</info> ", null);
		$namespace = $helper->ask($input, $output, $question);

		$question = new Question("Website URL <info>(Current installation URL)</info> ", null);
		$wp_url = $helper->ask($input, $output, $question);

		$question = new Question("Description? <info>(Imonweb WordPress Starter theme)</info> ", null);
	...
	{
		return getcwd().'/imon'.md5(time().uniqid()).'.zip';
	}

	/**
	 * Download the lates IMON release
	 *
	...
	{
		$response = $this->client->get('https://github.com/Imonweb/imon/archive/master.zip')->getBody();
	...
	private function extract($ZipFile, $directory, OutputInterface $output)
	{
		...
		if (! empty(shell_exec("mv ".escapeshellarg($directory)."/imon-master/* ".escapeshellarg($directory)."/imon-master/.[!.]*  ".escapeshellarg($directory)))) {
			$output->writeln('<comment>Unable to move files from master folder!</comment>');

			return $this;
		}

		rmdir($directory."/imon-master");
	...
	private function renameAllTheThings($directory, OutputInterface $output, $themeName = null, $namespace = null, $description = null, $wp_url = null, $themeURI = null, $author = null, $authorURI = null, $themeVersion = null)
	{
		...
		foreach ($file_info as $file) {
			...
			if (! is_null($namespace)) {
				$str = str_replace("imon", $namespace, $str);
				$str = str_replace("use Imon", "use " . $namespace, $str);
				$str = str_replace("namespace Imon", "namespace " . $namespace, $str);
				$str = str_replace("Imon\\", $namespace . "\\", $str);
				$str = str_replace("Imon\Init", $namespace . "\Init", $str);
			}

			if (! is_null($themeName)) {
				$str = str_replace("Imon", $themeName, $str);
			}
			
			if (! is_null($description)) {
				$str = str_replace("Imonweb WordPress Starter theme", $description, $str);
				$str = str_replace("Imonweb WordPress Starter Theme for modern developers", $description, $str);
			}
			...
			if (! is_null($themeURI)) {
				$str = str_replace("http://imonweb.co.uk/wp-starter", $themeURI, $str);
			}
			
			if (! is_null($author)) {
				$str = str_replace("Imon 'Imonweb' Dela Rosa", $author, $str);
			}
			
			if (! is_null($authorURI)) {
				$str = str_replace("http://imonweb.co.uk", $authorURI, $str);
			}
